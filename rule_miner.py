...
class RuleMiner(object):
    ...
    def get_confidence(self, data, rule):
        ...
        support1 = self.get_support(data, list(combined))
        support2 = self.get_support(data, rule[0])
    ...
    def get_association_rules(self, data):
        ...
        for itemset in itemsets:
            ...
            # class.
            rules2 = self.get_rules(itemset)
            rules.extend(rules2)

        # Extract rules where the right-hand side is 'HeartDisease'
        # heart_disease_rules = [rule for rule in rules if 'HeartDisease' in rule]

        # # Display the rules
        # print(heart_disease_rules)
